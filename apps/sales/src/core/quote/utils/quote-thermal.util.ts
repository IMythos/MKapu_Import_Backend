...
function calcHeight(quote: QuoteResponseDto): number {
  ...
  // Encabezado
  h += 12; // empresa
  if (quote.sede?.direccion) h += 9;
  if (quote.sede?.telefono)  h += 9;
  h += 8;  // divider

  // Nro cotización
  h += 12 + 9 + 9 + 9; // cod + emision + vence + estado
  h += 8;  // divider
  ...
  // Divider + totales
  h += 8 + 10 + 10 + 10; // =divider + 3 filas totales
  h += 8;  // divider
...
export function buildThermalPdf(quote: QuoteResponseDto): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    if (quote.sede?.direccion)
      line(quote.sede.direccion, { size: 6, align: 'center', gap: 9 });
    if (quote.sede?.telefono)
      line(`Cel: ${quote.sede.telefono}`, { size: 6, align: 'center', gap: 9 });

    divider('-');

    // ── Nro cotización ──────────────────────────────────────────────────────
    line(`COTIZACIÓN N° COT-${String(quote.id_cotizacion).padStart(6, '0')}`, {
      bold: true, size: 8, align: 'center', gap: 12,
    });
    line(`Emisión: ${new Date(quote.fec_emision).toLocaleDateString('es-PE')}`, { size: 6, align: 'center', gap: 9 });
    line(`Vence:   ${new Date(quote.fec_venc).toLocaleDateString('es-PE')}`,    { size: 6, align: 'center', gap: 9 });
    ...
    if (quote.cliente?.id_tipo_documento && quote.cliente?.valor_doc)
      line(`${quote.cliente.id_tipo_documento}: ${quote.cliente.valor_doc}`, { size: 6, gap: 9 });
    if (quote.cliente?.direccion)
      line(quote.cliente.direccion, { size: 6, gap: 9 });
    if (quote.sede?.nombre_sede)
      line(`Sede: ${quote.sede.nombre_sede} — ${quote.sede.ciudad}`, { size: 6, gap: 9 });
